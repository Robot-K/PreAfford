...
def train(conf, train_data_list, val_data_list):
    ...
    model_type = 'second'
    if conf.model_version == 'model_actor_fir':
    ...
    if model_type == 'first':
        network = model_def.Network(conf.feat_dim, conf.cp_feat_dim, conf.task_feat_dim,
                                    z_dim=conf.z_dim, lbd_kl=conf.lbd_kl, lbd_dir=conf.lbd_dir)
    else:
    ...
    train_dataset = SAPIENVisionDataset(conf.category_types, data_features, conf.train_buffer_max_num, \
                                        ...
                                        cat2freq=conf.cat2freq, model_type=model_type)

    val_dataset = SAPIENVisionDataset(conf.category_types, data_features, conf.val_buffer_max_num, \
                                      img_size=conf.img_size, no_true_false_equal=conf.no_true_false_equal, only_true_data=True, \
                                      coordinate_system=conf.coordinate_system, use_boxed_pc=conf.use_boxed_pc, \
                                      cat2freq=conf.val_cat2freq, model_type=model_type)
...
def forward(batch, data_features, network, conf, \
    ...
    if conf.model_version == 'model_actor_sec':
        ...
        losses = network.get_loss(pcs, ctpt1, dir1) 
        kl_loss = losses['kl']
        dir_loss = losses['dir']
        ...
        losses = network.get_loss(pcs, ctpt1, task)
        kl_loss = losses['kl']
        dir_loss = losses['dir']
        total_loss = losses['tot']
...
if __name__ == '__main__':
    ...
    for data_dir in offline_data_dir_list:
        train_data_list.extend(utils.append_data_list(data_dir, only_true_data=True))
    ...
    for data_dir in val_data_dir_list:
        val_data_list.extend(utils.append_data_list(data_dir, only_true_data=True))
