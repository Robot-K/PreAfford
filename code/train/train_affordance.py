...
def train(conf, train_data_list, val_data_list):
    ...
    model_type = 'second'
    if conf.model_version == 'model_aff_fir':
        data_features = ['part_pc', 'task', 'ctpt1', 'dir1', 'success', 'shape_id', 'result_idx', 'trajectory', 'score_before', 'score_after']
        model_type = 'first'
    ...
    train_dataset = SAPIENVisionDataset(conf.category_types, data_features, conf.train_buffer_max_num, \
                                        img_size=conf.img_size, no_true_false_equal=conf.no_true_false_equal, only_true_data=conf.only_true_data,
                                        coordinate_system=conf.coordinate_system, use_boxed_pc=conf.use_boxed_pc, \
                                        cat2freq=conf.cat2freq, model_type=model_type, succ_proportion=conf.succ_proportion, fail_proportion=conf.fail_proportion,
                                        expand_ctpt=True)

    val_dataset = SAPIENVisionDataset(conf.category_types, data_features, conf.val_buffer_max_num, \
                                      img_size=conf.img_size, no_true_false_equal=conf.no_true_false_equal, only_true_data=conf.only_true_data,
                                      coordinate_system=conf.coordinate_system, use_boxed_pc=conf.use_boxed_pc, \
                                      cat2freq=conf.val_cat2freq, model_type=model_type, succ_proportion=conf.succ_proportion, fail_proportion=conf.fail_proportion,
                                      expand_ctpt=True)
            ...
            while val_fraction_done <= train_fraction_done and val_batch_ind + 1 < val_num_batch:
                ...
                with torch.no_grad():
                    ...
                    val_loss = forward(batch=val_batch, data_features=data_features, network=network, conf=conf, is_val=True, \
                                 ...
                                 lr=network_opt.param_groups[0]['lr'], actor=actor, critic=critic)
                ep_loss += val_loss
                ep_cnt += 1
...
def forward(batch, data_features, network, conf, \
    ...
    if not conf.use_boxed_pc:
        input_pcid1 = torch.arange(batch_size).unsqueeze(1).repeat(1, conf.num_point_per_shape).long().reshape(-1)  # BN [0,0,...0,1,1,...1,2,2,...2,...] num_point_per_shape * batch_size
        input_pcid2 = furthest_point_sample(pcs, conf.num_point_per_shape).long().reshape(-1).to(conf.device)  # BN [a11,a12,a13,...,a21,a22,a23,...,am1,am2,am3,...] num_point_per_shape * batch_size
        pcs = pcs[input_pcid1, input_pcid2, :].reshape(batch_size, conf.num_point_per_shape, -1)

    ctpt = torch.tensor(np.array(batch[data_features.index('ctpt1')])).float().view(batch_size, -1).to(device)
    dir = torch.tensor(np.array(batch[data_features.index('dir1')])).float().view(batch_size, -1).to(device)

    if conf.model_version == 'model_aff_sec':
        pred_score = network.forward(pcs, ctpt)

        # get groundtruth
        with torch.no_grad():
            recon_dir = actor.actor_sample_n(pcs, ctpt, rvs=conf.rv_cnt).contiguous().view(batch_size * conf.rv_cnt, 6)
            gt_scores = critic.forward_n(pcs, ctpt, recon_dir, rvs=conf.rv_cnt)
            gt_score = gt_scores.view(batch_size, conf.rv_cnt, 1).topk(k=conf.topk, dim=1)[0].mean(dim=1)
        loss = network.get_loss(pred_score.view(batch_size, -1), gt_score.view(batch_size, -1))

    elif conf.model_version == 'model_aff_fir':
        pred_score = network.forward(pcs, ctpt)

        # get groundtruth
        with torch.no_grad():
            recon_dir1 = actor.actor_sample_n(pcs, ctpt, rvs=conf.rv_cnt).contiguous().view(batch_size * conf.rv_cnt, 3)
            gt_scores = critic.forward_n(pcs, ctpt, recon_dir1, rvs=conf.rv_cnt)  # recon_dir1: B*rvs, 3
            gt_score = gt_scores.view(batch_size, conf.rv_cnt, 1).topk(k=conf.topk, dim=1)[0].mean(dim=1)
        loss = network.get_loss(pred_score.view(batch_size, -1), gt_score.view(batch_size, -1))
...
if __name__ == '__main__':
    ...
    # train affordance
    parser.add_argument('--actor_version', type=str, help='model def file')
    parser.add_argument('--actor_path', type=str)
    parser.add_argument('--actor_eval_epoch', type=str)
    parser.add_argument('--critic_version', type=str, help='model def file')
    parser.add_argument('--critic_path', type=str)
    parser.add_argument('--critic_eval_epoch', type=str)
    ...
    parser.add_argument('--only_true_data', action='store_true', default=False)
    ...
    parser.add_argument('--bd_feat_dim', type=int, default=32)
    parser.add_argument('--bd_whole_feat_dim', type=int, default=64)
    parser.add_argument('--rv_dim', type=int, default=10)
    parser.add_argument('--rv_cnt', type=int, default=30)
    ...
    parser.add_argument('--succ_proportion', type=float, default=0.4)
    parser.add_argument('--fail_proportion', type=float, default=0.8)
    ...
    # if not conf.resume:
    #     os.system('cp datagen.py data_past.py models/%s.py %s %s' % (conf.model_version, __file__, conf.exp_dir))
    ...
    for data_dir in offline_data_dir_list:
        train_data_list.extend(utils.append_data_list(data_dir, only_true_data=conf.only_true_data))
    ...
    for data_dir in val_data_dir_list:
        val_data_list.extend(utils.append_data_list(data_dir, only_true_data=conf.only_true_data))
