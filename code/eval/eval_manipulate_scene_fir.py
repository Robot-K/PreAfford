...
parser.add_argument('--out_dir_final', type=str, default='xxx')
...
out_dir_final = args.out_dir_final
...
def save_succ_data(out_info, cam_XYZA_list, gt_link_mask, cam_XYZA_list_after, gt_link_mask_after, gif_imgs):
    ...
    utils.save_data(os.path.join(out_dir_final, 'total_files'), trial_id, out_info, cam_XYZA_list_after, gt_link_mask_after)
...
try:
    with open(os.path.join(out_dir, 'total_files', 'inferred_pre', f'result_{args.file_id}.json'), 'r') as fin:
...
# print(position1s)
task1s = inferred_data['task1s']
# print('dir1s = ', dir1s)
try:
    with h5py.File(os.path.join(out_dir, 'total_files', 'cambase', 'pre_XYZA_boxed_%d.h5' % args.file_id), 'r') as fin:
        # print(pc.shape)
        pc_center = fin['pc_center'][:].astype(np.float32)
except Exception:
    print('fail to load %s' % os.path.join(out_dir, 'total_files', 'cambase', 'pre_XYZA_boxed_%d.h5' % args.file_id))
    exit(5)

env.contact_error = None

num_pair = len(position1s)
num_pair = 1 # for debug
for trial in range(num_pair):
    ...
    task_world1 = np.array(task1s[trial]).reshape(3)
    given_left = np.array([0,0,-1])
    direct = task_world1 / np.linalg.norm(task_world1)
    ...
    pre_pose, pre_rotmat, start_pose, start_rotmat = utils.get_rotmat(cam, position_world1, given_up, number='1', out_info=out_info, start_dist=args.maneuver_dist, final_dist=args.start_dist, if_given_left=True, given_left=given_left)
    print(pre_pose, pre_rotmat, start_pose, start_rotmat)
    final_rotmat = start_rotmat.copy()
    print('start_rotmat', start_rotmat)
    final_rotmat[:2, 3] += task_world1[:2]
    ...
    con = utils.check_con(env, cam, after_cam_XYZA, scene, strict=False)
    success, traj_x, traj_y, traj_z = utils.check_success(trajectory, alpha, beta, gamma, task=task_world1, threshold=args.threshold, threshold_t=args.threshold_t, if_push=True, traj_robot = traj_robot, con=con, scene=scene)
    if con:
        success = True
