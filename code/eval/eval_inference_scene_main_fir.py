...
def run_jobs(idx_process, args, transition_Q, cur_file_list, out_dir):
    ...
    for file in cur_file_list:
        ...
        try:
            with h5py.File(os.path.join(out_dir, 'cambase', 'pre_XYZA_boxed_%d.h5' % file_id), 'r') as fin:
            ...
            print('fail to load %s' % os.path.join(out_dir, 'cambase', 'pre_XYZA_boxed_%d.h5' % file_id))
            continue
        ...
        with torch.no_grad():
            ...
            # print('actor1')
            task1s = actor1.actor_sample_n_diffCtpts(pc, position1s, rvs_ctpt=num_ctpt1, rvs=rv1).contiguous().view(batch_size * num_ctpt1 * rv1, 3)
            task1s[:, 2] = 0

            # print('critic1')
            critic_scores = critic1.forward_n_diffCtpts(pc, position1s, task1s, rvs_ctpt=num_ctpt1, rvs=rv1).view(batch_size, num_ctpt1 * rv1)
            ...
            task1s = task1s.view(batch_size, num_ctpt1 * rv1, 3)[batch_idx, selected_idx].view(batch_size * num_pair1, 3)
            pixel1_idx = selected_posi_idx[batch_idx, selected_idx // rv1]
            # print('start empty inference')
            position1s = position1s.detach().cpu().numpy()
            task1s = task1s.detach().cpu().numpy()
        ...
        for i in range(len(position1s)):
            cambase_batch = [position1s[i].reshape(3), task1s[i].reshape(3)]
            ...
            position_world1, task_world1 = world_batch
            position1s_world.append(position_world1.tolist())
            task1s_world.append(task_world1.tolist())
        result_data['position1s'] = position1s_world
        result_data['task1s'] = task1s_world
        with open(os.path.join(out_dir, 'inferred_pre', 'result_%d.json' % (file_id)), 'w') as fout:
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join(out_dir, dir_name, 'inferred_pre')):
        os.makedirs(os.path.join(out_dir, dir_name, 'inferred_pre'))
