...
class Network(nn.Module):
    def __init__(self, feat_dim, cp_feat_dim, task_feat_dim, hidden_feat_dim=128):
        super(Network, self).__init__()

        self.pointnet2 = PointNet2SemSegSSG({'feat_dim': feat_dim})

        self.critic = Critic(input_dim=feat_dim + cp_feat_dim + task_feat_dim , hidden_dim=hidden_feat_dim)

        self.mlp_task = nn.Linear(3, task_feat_dim)   # task
    ...
    def forward(self, pcs, cp, task):
        ...
        pred_result_logits = self.critic([net1, cp_feats,  task_feats])
        pred_scores = torch.sigmoid(pred_result_logits)
        return pred_scores

    def forward_n(self, pcs, cp, task, rvs=100):
        batch_size = len(cp)
        twos = (torch.ones(len(cp), 1)*1).to(cp.device)
        pcs[:, 0] = torch.cat((cp, twos), dim=1) #cp最后一维设成2
        # pcs = pcs[:, :, :3].repeat(1, 1, 2)
        ...
        pred_result_logits = self.critic([expanded_net1, expanded_cp1_feats, task_feats])
        pred_scores = torch.sigmoid(pred_result_logits)

        return pred_scores


    def forward_n_finetune(self, pcs, cp, task, rvs_ctpt=10, rvs=10):
        batch_size = pcs.shape[0]
        cp_feats = self.mlp_cp(cp)        # (B * rvs_ctpt) * dim
        task_feats = self.mlp_task(task)     # (B * rvs_ctpt * rvs) * dim
        ...
        pred_result_logits = self.critic([expanded_net,
                                          expanded_cp_feats, task_feats])     # (B * rvs_ctpt * rvs) * 1
        pred_scores = torch.sigmoid(pred_result_logits)

        return pred_scores


    def forward_n_diffCtpts(self, pcs, cp, task, rvs_ctpt=10, rvs=100):
        batch_size = pcs.shape[0]

        cp1_feats = self.mlp_cp(cp)        # (B * rvs_ctpt, -1)
        task_feats = self.mlp_task(task)     # (B * rvs_ctpt * rvs, -1)
