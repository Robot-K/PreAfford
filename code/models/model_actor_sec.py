...
class Network(nn.Module):
    def __init__(self, feat_dim, cp_feat_dim, dir_feat_dim, z_dim=128, lbd_kl=1.0, lbd_dir=1.0):
        ...
        self.all_encoder = ActorEncoder(input_dim=feat_dim + cp_feat_dim + dir_feat_dim, output_dim=z_dim)
        self.decoder = ActorDecoder(input_dim=feat_dim + cp_feat_dim + z_dim, output_dim=6)
    ...
    def forward(self, pcs, cp, dir):
        ...
        dir1_feats = self.mlp_dir(dir)

        z_all, mu, logvar = self.all_encoder([net1, cp1_feats, dir1_feats])
    ...
    def actor_sample(self, pcs, cp):
        ...
        twos = (torch.ones(len(cp), 1)).to(cp.device)
        pcs[:, 0] = torch.cat((cp, twos), dim=1) #cp最后一维设成2
        last_col = pcs[:, :, -1].unsqueeze(-1)
        pcs = torch.cat((pcs[:, :, :3].repeat(1, 1, 2), last_col), dim=2)
        whole_feats = self.pointnet2(pcs)
    ...
    def actor_sample_n_diffCtpts(self, pcs, cp, rvs_ctpt=10, rvs=100):
        ...
        return recon_dir1[:, :2, :] # batch_size * num_ctpt1 * rv1 , 2, 3
    ...
    def actor_sample_n_finetune(self, pcs, cp1, rvs_ctpt=10, rvs=100):
        ...
        z_all = torch.Tensor(torch.randn(batch_size * rvs_ctpt, rvs, self.z_dim)).to(pcs.device)
        twos = (torch.ones(len(cp1), 1)).to(cp1.device)
